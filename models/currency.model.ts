...
class Currency {
    ...
    static create(newCurrency: (string | number)[], result: any) {
        sql.query('INSERT INTO currencies SET ?', newCurrency, (err) => {
            if (err) {
    ...
    static createAll = (newCurrency: (string | number)[]) => {
        sql.query(
            'INSERT INTO currencies (cryptoName, coinbaseValue, coinstatsValue, coinmarketValue, ' +
            'coinpaprikaValue, kucoinValue, averagePrice) VALUES ?',
            [newCurrency],
            (err) => {
                if (err) {
                    console.log('error: ', err);
    ...
    static updateById = (id: number, currency: Currency, result: any) => {
        sql.query(
            'UPDATE currencies SET cryptoName = ?, coinbaseValue = ?, coinstatsValue = ?, coinmarketValue = ?, ' +
            'coinpaprikaValue = ?, kucoinValue = ?, averagePrice = ? WHERE id = ?',
            [
                currency.cryptoName,
